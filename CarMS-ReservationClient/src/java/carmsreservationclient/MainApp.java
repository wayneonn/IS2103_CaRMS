...
public class MainApp {
    ...
    private void doViewReservationDetails() throws RentalReservationNotFoundException {
        ...
        System.out.printf("%20s%25s%25s%13s%15s%30s%20s%20s\n", "Reservation ID", "Pickup Date & Time", "Return Date & Time", "Amount Paid", "Car Rented", "Dispatched Transit Driver", "Pickup Outlet", "Return Outlet");
        ...
        for (ReservationRecord reservation : reservations) {
            ...
            System.out.printf("%20s%25s%25s%13s%15s%30s%20s%20s\n", reservation.getReservationId().toString(), sdf.format(reservation.getPickupDateTime()), sdf.format(reservation.getReturnDateTime()), NumberFormat.getCurrencyInstance().format(reservation.getAmtPaid()), reservation.getCar().getLicenseNumber(), transitDriver, reservation.getPickupOutlet().getOutletName(), reservation.getReturnOutlet().getOutletName());
        }
...
//        scanner.nextLine();
    ...
    private void cancelReservation(ReservationRecord reservationRecord) {
